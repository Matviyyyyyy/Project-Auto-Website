...
@app.route("/filtred_year", methods=["POST"])
def filtred_year():
    sql_agent2 = SQLAgent(DB_NAME2)

    # Отримуємо значення радіо інпутів для року
    year_2021 = request.form.get("name_ra1")
    year_2022 = request.form.get("name_ra2")

    # Визначаємо рік
    year = 2021 if year_2021 else 2022 if year_2022 else None

    # Викликаємо функцію фільтрації за роком
    filter_car_year = sql_agent2.get_filtred_cars_year(year)

    # Передаємо відфільтровані автомобілі на сайт
    return render_template('chose-auto.html', all_brand_cars=filter_car_year)

@app.route("/filtred_drive_type", methods=["POST"])
def filtred_drive_type():
    sql_agent2 = SQLAgent(DB_NAME2)

    # Отримуємо значення радіо інпутів для типу приводу
    one_type_drive = request.form.get("name_ra3")
    two_type_drive = request.form.get("name_ra4")

    # Викликаємо функцію фільтрації за типом приводу
    filter_car_drive_type = sql_agent2.get_filtred_cars_drive_type(two_type_drive)

    # Передаємо відфільтровані автомобілі на сайт
    return render_template('chose-auto.html', all_brand_cars=filter_car_drive_type)
